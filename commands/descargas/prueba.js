...
export default {
  ...
  run: async (ctx) => {
    ...
    const userId = `${from}:video:legacy`;
    ...
    try {
      ...
      if (!rawInput) {
        ...
        return sock.sendMessage(from, {
          text: "❌ Uso: .ytmp4legacy <nombre o link> o responde a un mensaje con el link",
          ...global.channelInfo,
        });
      }
      ...
      await sock.sendMessage(
        ...
        thumbnail
          ? {
              image: { url: thumbnail },
              caption: `⬇️ Preparando video legacy...\n\n🎬 ${title}\n🎚️ Calidad: ${VIDEO_QUALITY}\n🌐 api dvyer`,
              ...global.channelInfo,
            }
          : {
              text: `⬇️ Preparando video legacy...\n\n🎬 ${title}\n🎚️ Calidad: ${VIDEO_QUALITY}\n🌐 api dvyer`,
              ...global.channelInfo,
            },
        quoted
      );
      ...
      rawVideoFile = path.join(TMP_DIR, `${stamp}-legacy-raw.mp4`);
      finalVideoFile = path.join(TMP_DIR, `${stamp}-legacy-final.mp4`);
      ...
      console.error("YTMP4 LEGACY ERROR:", err?.message || err);
      cooldowns.delete(userId);

      await sock.sendMessage(from, {
        text: `❌ ${String(err?.message || "Error al procesar el video legacy.")}`,
        ...global.channelInfo,
      });
    } finally {
      try {
